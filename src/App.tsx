      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-auto px-3 py-2 md:px-5 md:py-3">
          {displayMode === "signal_results_plate" ? (() => {
            ...
            /* ── Template 2: Compact 20-lifter flight-order table ── */
            <div className="flex h-full flex-col overflow-hidden">
              {/* Lift + Group label */}
              <div className={`shrink-0 px-3 py-1.5 border-b flex items-center justify-between ${isDarkTheme ? "border-white/10 bg-black/30" : "border-slate-200 bg-slate-50"}`}>
                <p className={`text-xs font-black uppercase tracking-[0.25em] ${isDarkTheme ? "text-cyan-300" : "text-cyan-800"}`}>
                  {currentLift.toUpperCase()} · ATTEMPT {currentAttemptIndex + 1}
                  {activeCompetitionGroupName && (
                    <span className={`ml-2 font-normal normal-case tracking-normal text-xs ${isDarkTheme ? "text-slate-400" : "text-slate-500"}`}>
                      — {activeCompetitionGroupName}
                    </span>
                  )}
                </p>
                <p className={`text-xs font-semibold tabular-nums ${isDarkTheme ? "text-slate-400" : "text-slate-500"}`}>
                  {flightLineOrdered.length} lifters
                </p>
              </div>
              {/* Table */}
              <div className="flex-1 overflow-hidden">
                <table className="w-full h-full" style={{ borderCollapse: "collapse", tableLayout: "fixed" }}>
                  ...
                  <thead>
                    <tr className={`text-left ${isDarkTheme ? "bg-[#0c0f1a] text-slate-400" : "bg-slate-100 text-slate-600"}`}>
                      {["#", "NAME", "WC", "BW", "LOT", "ATT 1", "ATT 2", "ATT 3"].map((col) => (
                        <th key={col} className="px-2 py-2 text-[11px] font-black uppercase tracking-widest truncate">{col}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {flightLineOrdered.slice(0, 20).map((lifter, idx) => {
                      const isCurrent = lifter.id === flightOrderHighlightId;
                      const atts = getAttempts(lifter, currentLift);
                      ...
                      return (
                        <tr
                          key={lifter.id}
                          className={`border-t ${isDarkTheme ? "border-white/[0.07]" : "border-black/[0.05]"} ${rowBg}`}
                        >
                          <td className={`px-2 tabular-nums font-black text-sm ${isCurrent ? (isDarkTheme ? "text-cyan-400" : "text-cyan-700") : isDarkTheme ? "text-slate-600" : "text-slate-400"}`}>{idx + 1}</td>
                          <td className={`px-2 font-semibold text-sm truncate ${isDarkTheme ? (isCurrent ? "text-white" : "text-slate-200") : "text-slate-900"}`}>
                            {isCurrent && <span className={`mr-1 text-[10px] font-black ${isDarkTheme ? "text-cyan-400" : "text-cyan-600"}`}>▶</span>}
                            {lifter.name || "—"}
                          </td>
                          <td className={`px-2 text-sm truncate ${isDarkTheme ? "text-slate-400" : "text-slate-500"}`}>{lifter.weightClass || "—"}</td>
                          <td className={`px-2 tabular-nums text-sm ${isDarkTheme ? "text-slate-400" : "text-slate-500"}`}>{typeof lifter.bodyweight === "number" ? lifter.bodyweight : "—"}</td>
                          <td className={`px-2 tabular-nums text-sm ${isDarkTheme ? "text-slate-500" : "text-slate-400"}`}>{typeof lifter.lot === "number" ? lifter.lot : "—"}</td>
                          {([0, 1, 2] as const).map((ai) => {
                            const att = atts[ai];
                            const w = att?.weight;
                            const s = att?.status ?? "UNATTEMPTED";
                            ...
                            const cls = s === "GOOD"
                              ...
                              : (isDarkTheme ? "text-slate-600" : "text-slate-400");
                            return (
                              <td key={ai} className={`px-2 tabular-nums text-sm ${cls}`}>{displayW}</td>
                            );
                          })}
                        </tr>
                      );
                    ...
                    {flightLineOrdered.length === 0 && (
                      <tr>
                        <td colSpan={8} className={`px-3 py-10 text-center text-sm ${isDarkTheme ? "text-slate-500" : "text-slate-400"}`}>
